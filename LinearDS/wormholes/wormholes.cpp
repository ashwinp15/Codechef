...
unsigned long long int takeTest (std::pair<int, int> testTimes[], int wormV[], int wormW[], int N, int X, int Y)
{
	 // initialising bestTime to the largest possible time
	 unsigned long long int bestTime = wormW[Y-1] - wormV[0] + 1;
	 for (int i=0; i<N; i++)
	 {
			int entry= 0;
			int exit = Y-1;
			...
			if ((testTimes[i].second - testTimes[i].first + 1) < bestTime)
			{
				 ...
				 unsigned long long int thisTime = wormW[exit] - wormV[entry] + 1;
